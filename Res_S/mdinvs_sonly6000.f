      ...
      IMPLICIT NONE
*
*     This routine reads singular matrices and computes model m
...
*
      ...
      CALL PDLAREAD( 'd.out', WORK( PTRA ), DESCA, 0, 0,
     $                WORK(PTRWORK) )
...
*     Update singular values and set zeros for elements under lowcut
      DO 30 I = 1, SIZE
         ...
         ELSE
            CALL PDELSET( WORK( PTRS ), I, I, DESCS, DBLE(1.0/SIGK) )
         ENDIF
   30 CONTINUE
*
...
*
*     R = V * Z
      CALL PDGEMM( 'T', 'N', N, N, N, ONE,
     $             WORK( PTRVT ), 1, 1, DESCVT,
     $             WORK( PTRS ), 1, 1, DESCS,
     $             ZERO, WORK( PTRR ), 1, 1, DESCR )
*     Z = U^T * G
      CALL PDGEMM( 'T', 'N', N, 1, M, ONE,
     $             WORK( PTRU ), 1, 1, DESCU,
     $             WORK( PTRA ), 1, 1, DESCA,
     $             ZERO, WORK( PTRS ), 1, 1, DESCS )
*     V = R * Z
      CALL PDGEMM( 'N', 'N', N, 1, N, ONE,
     $             WORK( PTRR ), 1, 1, DESCR,
     $             WORK( PTRS ), 1, 1, DESCS,
...
*
   ...
   !   $                0, 0, WORK(PTRWORK) )
   !    CALL PDLAWRITE( 'test.out', M, 1, WORK( PTRA ), IA, JA,
   !   $                DESCA, 0, 0, WORK(PTRWORK) )
      CALL PDLAWRITE( 'm6000.out', SIZE, 1, WORK( PTRVT ), IVT, JVT,
